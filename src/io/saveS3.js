...
const getFileKey = (userFolder, serviceName, stage, fileFolder, fileName) =>
  userFolder
    ? [userFolder, fileFolder, fileName].filter((e) => e).join('/')
    : [serviceName, stage, fileFolder, fileName].filter((e) => e).join('/');
...
const saveS3 = async (provider, s3Config, outputFiles) => {
  ...
  else if (failureCount === uploadResults.length)
    throw new Error('Failed to upload GraphQL requests to S3');
